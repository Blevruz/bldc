 ...
 *   <device>TimerDriver
 */
const CO_IF_TIMER_DRV ChOSTimerDriver = {
    DrvTimerInit,
    DrvTimerReload,
    DrvTimerDelay,
    DrvTimerStop,
...
static uint8_t DrvTimerUpdate(void)
{
    if (TimerCounter > 0u) TimerCounter--;
    return !TimerCounter;
}
...
static void DrvTimerStop(void)
{
	...
	// stopping timer must be done from comm/comm_can.c
   TimerCounter = 0;
}
